...
final tweetAPIProvider = Provider((ref) {
  return TweetAPI(
    ...
    realtime: ref.watch(tweetsAppwriteRealtimeProvider),
  );
});

abstract class ITweetAPI {
  FutureEither<Document> createTweet(Tweet tweet);
  Future<List<Document>> getTweets();
  Stream<RealtimeMessage> getLatestTweet();
  FutureEither<Document> likeTweet(Tweet tweet);
  FutureEither<Document> updateRetweetsCount(Tweet tweet);
...
class TweetAPI implements ITweetAPI {
  ...
  @override
  FutureEither<Document> createTweet(Tweet tweet) async {
    try {
  ...
  @override
  FutureEither<Document> updateRetweetsCount(Tweet tweet) async {
    try {
      final document = await _db.updateDocument(
        databaseId: AppwriteConstants.databaseId,
        collectionId: AppwriteConstants.tweetsCollection,
  ...
  Future<List<Document>> getUserTweets(String uid) async {
    final documents = await _db.listDocuments(
      databaseId: AppwriteConstants.databaseId,
      collectionId: AppwriteConstants.tweetsCollection,
      queries: [Query.equal('uid', uid), Query.orderDesc('tweetedAt')],
    );
    return documents.documents;
  }
